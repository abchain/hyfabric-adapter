...
type tags struct {
	...
	// rlp:"truncated" controls whether this field swallows additional
	// bytes. It can only be set for the last field, which must be
	// of []byte type.
	truncated bool
...
func parseStructTag(typ reflect.Type, fi, lastPublic int) (tags, error) {
	...
	for _, t := range strings.Split(f.Tag.Get("rlp"), ",") {
		...
		case "truncated":
			...
			if f.Type.Kind() != reflect.Slice && f.Type.Elem().Name() != "uint8" {
				return ts, structTagError{typ, f.Name, t, "field type is not []byte"}
			}
		default:
	...
	if ts.tail && ts.truncated {
		return ts, structTagError{typ, f.Name, "tail,truncated", "both tags can not be used simultaneously"}
	}
