...
func makeListDecoder(typ reflect.Type, tag tags) (decoder, error) {
	etype := typ.Elem()
	if etype.Kind() == reflect.Uint8 && !tag.truncated && !reflect.PtrTo(etype).Implements(decoderInterface) {
	...
	case tag.truncated:
		dec = func(s *Stream, val reflect.Value) error {
			...
			sz := int(s.remaining) + 9
			newv := reflect.MakeSlice(val.Type(), sz, sz)
			hsz := puthead(newv.Bytes(), 0xC0, 0xF7, s.remaining)
			val.Set(newv)
			val.SetLen(int(s.remaining) + hsz)
			if err := s.readFull(val.Bytes()[hsz:]); err != nil {
				return err
			}
